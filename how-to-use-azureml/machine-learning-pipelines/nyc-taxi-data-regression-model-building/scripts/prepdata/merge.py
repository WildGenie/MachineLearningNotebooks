...
print(f"Argument (output merge taxi data path): {args.output_merge}")

# Appending yellow data to green data
combined_df = green_df.append(yellow_df, ignore_index=True)
combined_df.reset_index(inplace=True, drop=True)

if args.output_merge is not None:
    os.makedirs(args.output_merge, exist_ok=True)
    print(f"{args.output_merge} created")
    path = f"{args.output_merge}/processed.parquet"
